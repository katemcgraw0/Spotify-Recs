...
const artistRecs = () => {
  ...
  useEffect(() => {
    const generateRecs = async () => {
      try {
        const prompt = makePromptArtistRecs(artists.map(artist => artist.name))
        ...
        const artistRecsArray = data.result
          .split(/\d+\./)
        ...
        {artistPic.length > 0 && artistPic.length === artistRecs.length && (
          <div className='flex flex-col items-center justify-center text-black'>
            ...
            <div className='flex justify-center mt-8'>
              <img
                src='SpotifyLogo.png' // Replace with the path to your Spotify logo image
                alt='Spotify Logo'
