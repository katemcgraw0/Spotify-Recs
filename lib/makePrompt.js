...
export function makePromptSongRecs (favArtists, favGenres) {
  const separator = ', '
  const artistString = favArtists.slice(0, 3).join(separator)
  const genresString = favGenres.slice(0, 5).join(separator)

  const intro = `My favorite music artists are: `
  const songintro = `My favorite genres are: `
  const request = `Please generate 4 new song recommendations in the following format, with no extra text. Do not recommend songs of these artists.: 
  ...
  '`
  return `${intro} ${artistString} ${songintro} ${genresString} ${request}`
}
